...
export default function ProductForms({product}:{product:any}){
  ...
  const [selectedForm,setSelectedForm] = useState<{key:string;label:string;size?:string} | null>(null)
  ...
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {renderForms.map(f=> (
          <FormCard key={f.key} form={f} product={product} onClick={()=>openForm(f)} />
        ))}
      </div>

      <ProductModal product={product} open={open} onClose={()=>{setOpen(false); setSelectedForm(null)}} form={selectedForm ?? undefined} />
    </div>
  )
}
